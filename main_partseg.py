...
from model9 import DGCNN_partseg
import numpy as np
...
def train(args, io):
    ...
    for epoch in range(args.epochs):
        ...
        outstr = 'Best %d, test iou: %.6f' % (best_epoch,
                                              best_test_iou)
        io.cprint(outstr)
        print(outstr)
...
def test(args, io):
    test_loader = DataLoader(ShapeNetPart(partition='test', num_points=args.num_points, class_choice=
                    args.class_choice
                                          ),
    ...
    for data, label, seg in test_loader:
        ...
        #np_array_list = np.load('my_list.npy', allow_pickle=True)
        #feature1, feature2, feature3, cloud = [i for i in np_array_list]
        #label = torch.tensor([[4]], dtype=torch.int32)
        #data = torch.tensor([cloud], dtype=torch.float32)
        #data = data.permute(0, 2, 1)
        seg = seg - seg_start_index
        label_one_hot = np.zeros((label.shape[0], 16))
        for idx in range(label.shape[0]):
        ...
        # for seg_id in np.unique(pred_np[0]):
        #     mask = pred_np[0] == seg_id
        #     #color_array[mask] = colors[seg_id]
        #     mlab.points3d(cloud[mask, 0], cloud[mask, 1], cloud[mask, 2],
        #                   color=colors[seg_id-np.min(np.unique(pred_np[0]))], mode='sphere', figure=fig, scale_factor=0.04)
        #
        # mlab.show()
        # seg = seg[0].detach().cpu().numpy()
        # #random_indices = np.random.choice(2048, 512, replace=False)
        #
        # #cloud = cloud[random_indices]
        # #seg = seg[random_indices]
        # fig = mlab.figure(bgcolor=(1, 1, 1))
        # for seg_id in np.unique(seg):
        #     mask = seg == seg_id
        #     # color_array[mask] = colors[seg_id]
        #     mlab.points3d(cloud[mask, 0], cloud[mask, 1], cloud[mask, 2],
        #                   color=colors[seg_id-np.min(np.unique(seg))], mode='sphere', figure=fig, scale_factor=0.04)
        #
        # mlab.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_name', type=str, default='model9_', metavar='N',
    ...
    parser.add_argument('--batch_size', type=int, default=26, metavar='batch_size',
                        help='Size of batch)')
    ...
    parser.add_argument('--model_path', type=str, default='outputs/model9/models/model.t7', metavar='N',
