...
class ScanNet(Dataset):
    def __init__(self, num_point=8192, partition='train', 
        ...
        xyz_all = xyz_all[0][:10]
        #xyz_all = np.concatenate([np.array(cloud) for cloud in xyz_all[0][:1]], axis=0)
        #xyz_all = np.hstack(xyz_all)
        label_all = label_all[0][:10]
        #label_all = np.concatenate([np.array(label) for label in label_all[0][:1]], axis=0)
